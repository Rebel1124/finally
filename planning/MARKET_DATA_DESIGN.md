...
**Timestamp units — convert defensively.** Unix **nanoseconds** in the raw REST payload for
`last_trade`, but the Python client's `last_trade.timestamp` attribute is
documented/observed as **milliseconds**. `MassiveDataSource` divides by 1000 assuming
milliseconds; if this ever looks wrong in practice, sanity-check the result against
`time.time()` rather than assuming a fixed unit blindly.
