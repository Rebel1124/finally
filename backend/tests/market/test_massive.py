...
class TestMassiveDataSource:
    ...
    async def test_poll_updates_cache_using_real_sdk_models(self):
        """Test against real massive.rest.models objects, not MagicMock.

        Guards against attribute-name drift between our parsing code and the
        installed SDK's actual model shape (MagicMock accepts any attribute
        name, so a mocked-only test suite can't catch a renamed/missing field).
        """
        ...
        with patch.object(source, "_fetch_snapshots", return_value=[real_snapshot]):
            await source._poll_once()
