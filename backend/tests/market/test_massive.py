...
def make_snapshot(ticker: str, price: float, timestamp_ms: float):
...
async def test_start_performs_immediate_poll(monkeypatch, cache, source):
    monkeypatch.setattr(
        source,
        "_fetch_snapshots",
        lambda: [make_snapshot("AAPL", 190.5, 1_700_000_000_000)],
    )
...
async def test_poll_once_updates_cache_with_converted_timestamp(cache, source):
    ...
    monkeypatch_snapshot = [make_snapshot("AAPL", 190.5, 1_700_000_000_000)]
...
async def test_poll_once_skips_malformed_snapshot_without_crashing(cache, source):
    ...
    good = make_snapshot("AAPL", 190.5, 1_700_000_000_000)
...
async def test_poll_loop_polls_repeatedly(monkeypatch, cache, source):
    ...
    def _fetch():
        nonlocal call_count
        call_count += 1
        return [make_snapshot("AAPL", 100.0 + call_count, 1_700_000_000_000)]
